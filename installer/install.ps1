...
trap {
  ...
  Write-Host 'This run may have written the following paths (preserved for diagnosis; no client config was changed):'
...
if (-not $PythonExe) {
    foreach ($candidate in @('py.exe','python.exe')) {
        ...
        if ($cmd) {
            if ($candidate -eq 'py.exe') { $found=& $cmd.Source -3 -c 'import sys;print(sys.executable)' 2>$null } else { $found=& $cmd.Source -c 'import sys;print(sys.executable)' 2>$null }
...
if ($DiagnosticOnly) {
  ...
  try { $scripts=& $PythonExe -B (Join-Path $PSScriptRoot 'verify-dependency.py'); if($LASTEXITCODE -ne 0){throw 'Dependency check failed'}; $mcp=Join-Path $scripts.Trim() 'autocad-mcp.exe'; Add-Result 'AutoCAD MCP Pro' 'PASS' $mcp }
  ...
  & $PythonExe -B (Join-Path $PSScriptRoot 'verify-rules.py') --root (Join-Path $InstallRoot 'pack-rules') --packed
  Add-Result 'Rule integrity' $(if($LASTEXITCODE -eq 0){'PASS'}else{'FAIL'}) 'Installed manifest and all rule hashes checked'
  $run=$null
  $diagFail=@($Results|Where-Object{$_.Status -eq 'FAIL'}).Count
  Show-Summary; if($diagFail -gt 0){exit 1} else {exit 0}
}
New-Item -ItemType Directory -Path $InstallRoot -Force | Out-Null
$written.Add($InstallRoot)
if(-not $DiagnosticOnly -and (Test-Path -LiteralPath (Join-Path $InstallRoot 'pack-rules'))){
  & $PythonExe -B (Join-Path $PSScriptRoot 'verify-rules.py') --root (Join-Path $InstallRoot 'pack-rules') --packed
  if($LASTEXITCODE -ne 0){ Add-Result 'Existing rule integrity' 'FAIL' 'Existing rules preserved; do not overwrite corruption'; Show-Summary; exit 1 }
  $installed=Get-Content -LiteralPath (Join-Path $InstallRoot 'pack-rules\manifest.json') -Raw | ConvertFrom-Json
  $candidate=Get-Content -LiteralPath (Join-Path $package 'manifest.json') -Raw | ConvertFrom-Json
  if($installed.release_version -ne $candidate.release_version){ throw 'Different installed version preserved. Select a separate InstallRoot for this candidate.' }
  Add-Result 'Installed version' 'PASS' ('Same version already installed: '+$installed.release_version+'; rechecking smoke')
}
...
if (-not $UseExistingDependency) {
    ...
    if($installed){
        & $PythonExe -B (Join-Path $PSScriptRoot 'verify-dependency.py')
        if($LASTEXITCODE -ne 0){throw 'Existing runtime dependency check failed; preserved. Use separate InstallRoot.'}
    } else {
        & $PythonExe -m pip install -r (Join-Path $PSScriptRoot 'requirements.txt')
        if ($LASTEXITCODE -ne 0) { throw 'Dependency installation failed: see pip output; written runtime preserved for diagnosis.' }
    }
}
...
if (Test-Path -LiteralPath $target) {
    ...
    foreach($file in Get-ChildItem -LiteralPath $source -Recurse -File){
        ...
        if(-not(Test-Path -LiteralPath $other) -or (Get-PackHash $file.FullName) -ne (Get-PackHash $other)){$same=$false}
    }
    if($same){ Add-Result 'Skill' 'PASS' 'Identical Skill already installed; no duplicate created' }
    else { Add-Result 'Skill' 'WARN' 'Existing different Skill preserved; candidate staged under InstallRoot/skills; client discovery requires manual review'; $target=Join-Path $InstallRoot 'skills\sketchup-from-cad-landscape' }
}
...
foreach ($rel in $ruleFiles) {
  ...
  $release=Get-Content -LiteralPath (Join-Path $package 'manifest.json') -Raw | ConvertFrom-Json
  $manifestFiles += $release.files | Where-Object { $_.relative_path -eq $rel.Replace('\','/') }
}
[ordered]@{ schema_version=1; release_version=$release.release_version; required_directories=@('standards','guards','docs'); files=$manifestFiles } | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $packRules 'manifest.json') -Encoding UTF8
...
if($vrCode -ne 0){ Show-Summary; exit 1 }
...
Add-Result 'Client reload' 'WARN' 'MANUAL TEST REQUIRED: config fragments only; live client config untouched'
Show-Summary
if ($smokeExit -ne 0) { Write-Host ('FAIL: writes retained for diagnosis: '+($written -join ', ')); exit $smokeExit }
exit 0
