...
m=json.loads((root/'manifest.json').read_text());dest=out/'Landscape-Agent-Pack-v0.1.0-beta-candidate.1.zip'
with zipfile.ZipFile(dest,'w',zipfile.ZIP_DEFLATED) as z:
    for rel in [e['relative_path'] for e in m['files']]+['manifest.json','SHA256SUMS.txt']:
        z.write(root/rel,'Landscape-Agent-Pack-v0.1.0-beta-candidate.1/'+rel)
extract=out/'candidate-extracted'
with zipfile.ZipFile(dest) as z:z.extractall(extract)
...
fail=v.verify(extract/'Landscape-Agent-Pack-v0.1.0-beta-candidate.1')
